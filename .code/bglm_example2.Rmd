...
```{r fitModel2j1, results='markdown', eval=TRUE, echo = FALSE, hidden=TRUE}
save(polis.brm3, file = '../ws/testing/polis.brm3.RData')
```


```{r fitModel2j2, results='markdown', eval=TRUE, hidden=TRUE, cache=FALSE}
...
```{r LD501a, results='markdown', eval=TRUE, hidden=TRUE, fig.width=8, fig.height=5}
polis.rstanarm3 %>% tidy_draws() %>%
...
```{r LD502a, results='markdown', eval=TRUE, hidden=TRUE, fig.width=8, fig.height=5}
polis.brm3 %>% tidy_draws() %>%
...
```{r figureModel2b, results='markdown', eval=TRUE, hidden=TRUE}
## Using emmeans
polis.grid = with(polis, list(RATIO = seq(min(RATIO), max(RATIO), len=100)))
