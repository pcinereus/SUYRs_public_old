...
```{r fitModel1g1, results='markdown', eval=TRUE, hidden=TRUE, cache=FALSE}
quinn.rstanarm1 %>%
...
```{r fitModel1g2, results='markdown', eval=TRUE, hidden=TRUE, cache=FALSE}
quinn.rstanarm1 %>%
...
```{r fitModel1i1, results='markdown', eval=TRUE, hidden=TRUE, cache=FALSE}
quinn.rstanarm2 %>%
    ggpredict(~SEASON+DENSITY) %>%
    plot(add.data = TRUE)
quinn.rstanarm2 %>%
...
```{r fitModel1i2, results='markdown', eval=TRUE, hidden=TRUE, cache=FALSE}
quinn.rstanarm2 %>%
    ggemmeans(~SEASON+DENSITY) %>%
    plot(add.data=TRUE) %>%
    plot(add.data = TRUE) %>%
...
```{r modelFit1l1, results='markdown', eval=TRUE, hidden=TRUE, fig.width=6, fig.height=4}
quinn.rstanarm3 %>%
    ggpredict(~SEASON+DENSITY) %>%
    plot(add.data = TRUE)
```
...
```{r modelFit1l2, results='markdown', eval=TRUE, hidden=TRUE, fig.width=6, fig.height=4}
quinn.rstanarm3 %>%
    ggemmeans(~SEASON+DENSITY) %>%
    plot(add.data=TRUE) 
```
...
```{r fitModel2a1, results='markdown', eval=TRUE, hidden=TRUE, cache=TRUE, echo=2}
options(width=100)
...
```{r fitModel2k2, results='markdown', out.width = 600, fig.width = 8, fig.height = 4, eval=TRUE, hidden=TRUE, cache=FALSE}
...
```{r fitModel3a, results='markdown', eval=TRUE, hidden=TRUE, cache=TRUE}
...
```{r fitModel3b2, results='markdown', eval=TRUE, hidden=TRUE, cache=FALSE}
quinn.rstanarmNB %>% ggpredict(~SEASON+DENSITY) %>% plot(add.data = TRUE)
```

### ggemmeans

```{r fitModel3b3, results='markdown', eval=TRUE, hidden=TRUE, cache=FALSE}
quinn.rstanarmNB %>%
...
```{r fitModel3b4, results='markdown', eval=TRUE, hidden=TRUE, cache=FALSE}
...
### DHARMa residuals
```{r fitModel3b5, results='markdown', eval=TRUE, hidden=TRUE, cache=FALSE, fig.width = 8, fig.height = 4, out.width = 600}
...
```{r fitModel3c, results='markdown', eval=TRUE, hidden=TRUE, cache=TRUE}
...
### Fit model
```{r fitModel4a, results='markdown', eval=TRUE, hidden=TRUE, cache=TRUE}
...
```{r fitModel4a1, results='markdown', eval=TRUE, hidden=TRUE, cache=FALSE, fig.width = 8, fig.height = 4, out.width = 600}
...
```{r summariseModel1c8, results='markdown', eval=TRUE, hidden=TRUE, fig.width=8, fig.height=5}
...
```{r summariseModel2d5, results='markdown', eval=TRUE, hidden=TRUE, fig.width=8, fig.height=5,echo=TRUE}
## Link scale
quinn.rstanarmNB %>%
    gather_draws(`.Intercept.*|.*SEASON.*|.*DENSITY.*`, regex=TRUE) %>%
    ggplot() +
...
```{r summariseModel1c44, results='markdown', eval=TRUE, hidden=TRUE, fig.width=8, fig.height=5,echo=TRUE}
quinn.rstanarmNB %>% 
  gather_draws(`.Intercept.*|.*SEASON.*|.*DENSITY.*`, regex=TRUE) %>% 
  ggplot() + 
  stat_halfeye(aes(x=.value,  y=.variable)) +
...
```{r summariseModel2c45, results='markdown', eval=TRUE, hidden=TRUE, fig.width=8, fig.height=5,echo=TRUE}
quinn.brmsNB %>%
    gather_draws(`.Intercept.*|.*SEASON.*|.*DENSITY.*`, regex=TRUE) %>% 
    ggplot() +
    geom_vline(xintercept=0, linetype='dashed') +
...
We could alternatively express the parameters on the response scale.
```{r summariseModel2c55, results='markdown', eval=TRUE, hidden=TRUE, fig.width=8, fig.height=5,echo=TRUE}
quinn.brmsNB %>% 
  gather_draws(`.Intercept.*|.*SEASON.*|.*DENSITY.*`, regex=TRUE) %>%
  group_by(.variable) %>%
  mutate(.value=exp(.value)) %>%
...
```{r summariseModel2d55, results='markdown', eval=TRUE, hidden=TRUE, fig.width=8, fig.height=5,echo=TRUE}
## Link scale
quinn.brmsNB %>%
    gather_draws(`.Intercept.*|.*SEASON.*|.*DENSITY.*`, regex=TRUE) %>%
    ggplot() +
...
```{r summariseModel2k, results='markdown', eval=TRUE, hidden=TRUE, fig.width=8, fig.height=5}
...
### Simple
```{r mainEffects2a, results='markdown', eval=TRUE, hidden=TRUE}
quinn.brmsNB %>%
    emmeans(~DENSITY|SEASON, type='response') %>%
    pairs()
## absolute response scale
...
```{r mainEffects2b, results='markdown', eval=TRUE, hidden=TRUE}
...
```{r summaryFig1a, results='markdown', eval=TRUE, hidden=TRUE, fig.width=8, fig.height=4}
...
```{r summaryFig2a, results='markdown', eval=TRUE, hidden=TRUE, fig.width=8, fig.height=4}
...
# Observation-level random effects {.tabset .tabset-faded}	
## brms

```{r fitModel.brms, results='markdown', eval=TRUE, hidden=TRUE}
quinn <- quinn %>%
  group_by(SEASON, DENSITY) %>%
