...
```{r fitModel2a, results='markdown', eval=TRUE, hidden=TRUE, cache=TRUE, paged.print=FALSE, tidy.opts = list(width.cutoff = 80), echo=c(-4,-6)}
...
```{r fitModel2h, results='markdown', eval=TRUE, hidden=TRUE, cache=FALSE, fig.width = 10, fig.height = 7}
tobacco %>% 
    group_by(TREATMENT) %>%
...
```{r summariseModel2i2, results='markdown', eval=TRUE, hidden=TRUE, fig.width=8, fig.height=5}
tobacco.brm3 %>%
  summarise_draws(
    median,
    ~ HDInterval::hdi(.x),
...
```{r predictions2a, results='markdown', eval=TRUE, hidden=TRUE, fig.width=8, fig.height=5}
tobacco.brm3 %>% emmeans(~TREATMENT) %>%
    gather_emmeans_draws() %>%
    group_by(.draw) %>%
    arrange(desc(TREATMENT)) %>%
