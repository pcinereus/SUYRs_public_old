...
We should start by declaring any categorical variables (including an varying
...
```{r fitModel1j2, results='markdown', eval=TRUE, hidden=TRUE, cache=TRUE, dependson='fitModel1h'}
...
```{r posteriors1b2, results='markdown', eval=TRUE, hidden=TRUE}
norin.em <- norin.brm4 %>%
    emmeans(~TRIAL|SMR_contr, at=norin.grid) %>%
    pairs() %>% 
    gather_emmeans_draws() %>%
