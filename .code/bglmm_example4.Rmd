...
```{r fitModel1g3, results='markdown', eval=TRUE, hidden=TRUE, cache = FALSE}
...
- $\beta_1$: normal centred at 0 with a standard deviation of 0.5
  - sd of 0.5: since `model.matrix(~SYMBIONT, data=mckeon)[,-1] %>% apply(2,sd)` 
- $\sigma_j$: half-cauchy with parameters 0 and 1.
- $\Sigma$: decov with:
...
```{r fitModel10, results='markdown', eval=TRUE, hidden=TRUE, cache=FALSE}
mckeon.brm2 %>%
    ggpredict(~SYMBIONT) %>%
    plot(add.data = TRUE)
```
...
```{r fitModel1j1, results='markdown', eval=TRUE, hidden=TRUE, cache=TRUE, dependson='fitModel1h'}
