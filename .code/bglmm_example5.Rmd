...
- for the intercept (when the family is Gaussian), a normal prior with a mean of 0 and a
...
- $\beta_1$: normal centred at 0 with a standard deviation of 5
- $\Sigma$: decov with:
...
- $\beta_0$: normal centred at 1.8 with a standard deviation of 5
...
```{r fitModel6, results='markdown', eval=TRUE, hidden=TRUE, cache=TRUE}
priors <- prior(normal(1.8, 5), class='Intercept') +
    prior(normal(0, 5), class='b') +
    prior(cauchy(0,1), class='sd') +
    prior(logistic(0,1), class='Intercept', dpar='zi') +
...
```{r fitModel5, results='markdown', eval=TRUE, hidden=TRUE, cache=TRUE}
priors <- prior(normal(1.8, 5), class='Intercept') +
    prior(normal(0, 5), class='b') +
    prior(cauchy(0,1), class='sd') +
    prior(logistic(0,1), class='Intercept', dpar='zi') +
...
```{r fitModel7a, results='markdown', eval=TRUE, hidden=TRUE, fig.width=7, fig.height=4}
(l.1 <- loo(owls.brm3))
(l.2 <- loo(owls.brm4))
(l.3 <- loo(owls.brm5))
(l.4 <- loo(owls.brm6))
...
```{r fitmodelrstanarm, results='markdown', eval=FALSE,hidden=TRUE, echo=FALSE}
...
```{r fitModel.brms, results='markdown', eval=FALSE, hidden=TRUE, echo=FALSE}
owls.form <- bf(NCalls ~ FoodTreatment*SexParent +
                    offset(log(BroodSize)) + (1|Nest),
                family=poisson(link='log'))
