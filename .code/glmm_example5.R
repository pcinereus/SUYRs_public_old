...
owls.glmmTMB4a <- update(owls.glmmTMB4, .~. + FoodTreatment*SexParent, REML = FALSE)
owls.glmmTMB4b <- update(owls.glmmTMB4, .~. + FoodTreatment+SexParent, REML = FALSE)
...
##owls.resid %>% testTemporalAutocorrelation(time=owls$ArrivalTime)
## owls.resid1 <- owls.resid %>%
##     recalculateResiduals(group=owls$ArrivalTime,
##                          aggregateBy = mean)
## owls2 <- owls %>%
##     group_by(ArrivalTime) %>%
##     summarise(ArrivalTime = mean(ArrivalTime))
## owls.resid1 %>% testTemporalAutocorrelation(time=owls2$ArrivalTime)
owls2 <- owls %>%
    mutate(TIME = as.numeric(Nest) + ArrivalTime/100 + rnorm(n(),0,0.001))
owls.resid %>% testTemporalAutocorrelation(time=owls2$TIME)
## owls2 <- owls %>%
##     mutate(TIME = numFactor(Nest, ArrivalTime+ rnorm(n(),0,0.001)))
## owls.resid %>% testTemporalAutocorrelation(time=owls2$TIME)

owls.glmmTMB5 <- owls.glmmTMB4a %>% update(.~.+ou( 0 + numFactor(ArrivalTime) | Nest))
owls.glmmTMB9 <- owls.glmmTMB4a %>% update(.~.+ar1( 0 + numFactor(ArrivalTime) | Nest),
                                           family=nbinom2(),
                                           ziformula = ~ SexParent * FoodTreatment)
## owls.glmmTMB5 <- owls.glmmTMB4a %>% update(
##                                            family=nbinom2(),
##                                            ziformula = ~ SexParent * FoodTreatment)
## owls.glmmTMB5 <- owls.glmmTMB4a %>% update(.~.+ar1( 0 + numFactor(ArrivalTime) | Nest),
##                                            control = glmmTMBControl(optimizer = 'optim',
##                                                                     optArgs = 'Nelder-Mead'))
owls.resid <- owls.glmmTMB5 %>% simulateResiduals(plot=TRUE, integerResponse = TRUE)
owls.resid %>% testUniformity()
owls.resid <- owls.glmmTMB4a %>% simulateResiduals(plot=TRUE, integerResponse = TRUE)
owls.glmmTMB5 %>% summary()
...
owls.resid <- owls.glmmTMB7 %>% simulateResiduals(plot=TRUE, integerResponse = TRUE)
owls.resid %>% testZeroInflation()
owls.resid %>% testDispersion()
owls.resid %>% testUniformity()
owls.resid %>% testQuantiles()
...
owls.resid <- owls.glmmTMB8 %>% simulateResiduals(plot=TRUE, integerResponse = TRUE)
owls.resid %>% testZeroInflation()
owls.resid %>% testDispersion()
owls.resid %>% testUniformity()
owls.resid %>% testQuantiles()
...
AICc(owls.glmmTMB4a, owls.glmmTMB5,  owls.glmmTMB6, owls.glmmTMB7, owls.glmmTMB8, owls.glmmTMB9)
...
owls.glmmTMB9 %>% ggemmeans(~FoodTreatment+SexParent, offset=log(off$Mean)) %>% plot()


## ----partialPlots1d2, results='markdown', eval=TRUE---------------------------
owls.glmmTMB8 %>% ggemmeans(~FoodTreatment+SexParent, offset=0) %>% plot()
owls.glmmTMB9 %>% ggemmeans(~FoodTreatment+SexParent, offset=0) %>% plot()
...
## owls.glmmTMB9 %>% performance::r2_nakagawa()
